...
public class HsmFieldParser {
    ...
    public List<IHsmField> parse(ByteBuffer data) {
        ...
        if (result.size() < 3) {
            logger.warn("parse fields size < 2. rece data is {}.", HexUtil.toHexString(data.array()));
            return null;
            ...
            logger.warn("parse has a unKnow error. rece data is {}. ", HexUtil.toHexString(data.array()));
            return null;
        }
    }
}
